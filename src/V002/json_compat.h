...
// ArduinoJson v6와 v7을 모두 지원하기 위한 호환 계층
  ...
  inline void jSerialize(const JsonDoc& doc, String& out) { serializeJson(doc, out); }
  inline void jSerialize(const JsonDoc& doc, Stream& out) { serializeJson(doc, out); }
  template<typename T>
  inline T jAs(const JsonVariantConst& v) { return v.as<T>(); }
  inline JsonArray jToArray(JsonDoc& d) { return d.to<JsonArray>(); }
#else
