...
void initWiFiManagerInstance() { wifiManager.setDebugOutput(true); }
...
bool saveWiFiCredentialsToNVS(const String& ssid, const String& password, int priority) {
  ...
  // 정렬
  struct NI { String s; String p; int pr; };
  std::vector<NI> v;
  for (JsonObject n : wifiList) {
...
void syncCurrentWiFiToStorage() {
  ...
  for (JsonObject o : arr) { o["priority"] = (o["priority"] | 0) + 1; }
...
void resetWiFiSettings() { wifiManager.resetSettings(); }
bool resetWiFi() { resetWiFiSettings(); return true; }

// WiFiManager 비번 동기화
bool updateSavedWiFiPasswordFromWiFiManager(const String& ssid, WiFiManager& wm) {
  String connectedSSID = WiFi.SSID();
  String pass;
  if (connectedSSID == ssid) pass = wm.getWiFiPass(true);
  else if (wm.getWiFiSSID(true) == ssid) pass = wm.getWiFiPass(true);
  ...
  if (jDeserialize(doc, json)) {
    ...
    o["ssid"] = ssid; o["password"] = pass; o["priority"] = 1;
  } else {
    bool found = false;
    JsonArray a = doc.as<JsonArray>();
    for (JsonObject n : a) {
      if (n["ssid"] == ssid) { n["password"] = pass; found = true; break; }
    }
    if (!found) {
      JsonObject o = a.createNestedObject();
      o["ssid"] = ssid; o["password"] = pass; o["priority"] = a.size()+1;
    }
  }
  String updated;
  jSerialize(doc, updated);
...
// 전역 오버로드
bool   updateSavedWiFiPasswordFromWiFiManager(const String& ssid) { return updateSavedWiFiPasswordFromWiFiManager(ssid, wifiManager); }
String getWiFiPasswordFromWiFiManager(const String& ssid) { return getWiFiPasswordFromWiFiManager(ssid, wifiManager); }
...
void handleAddWifiNetworkCommand(AsyncWebSocketClient* client, JsonDoc& doc) {
  ...
  // NVS 읽기
  Preferences prefs;
  if (!prefs.begin(WIFI_NAMESPACE, true)) { sendErrorResponse(client, "NVS open failed"); return; }
  String json = prefs.getString(WIFI_CREDENTIALS_KEY, "[]");
  prefs.end();

  JsonDoc wdoc(WIFI_CREDENTIALS_JSON_SIZE);
  if (jDeserialize(wdoc, json)) { wdoc.clear(); jToArray(wdoc); }
  JsonArray nets = wdoc.is<JsonArray>() ? wdoc.as<JsonArray>() : jToArray(wdoc);

  bool exists=false;
  for (JsonObject n : nets) {
    if (n["ssid"].as<String>() == ssid) { n["password"]=pwd; n["priority"]=priority; exists=true; break; }
  }
  if (!exists) { JsonObject n = nets.createNestedObject(); n["ssid"]=ssid; n["password"]=pwd; n["priority"]=priority; }

  String updated; jSerialize(wdoc, updated);
  bool ok=false;
  if (prefs.begin(WIFI_NAMESPACE, false)) { ok = prefs.putString(WIFI_CREDENTIALS_KEY, updated); prefs.end(); }

  JsonDoc resp(128);
  resp["action"]="addWifiNetworkResponse"; resp["success"]=ok; resp["ssid"]=ssid;
  String out; jSerialize(resp, out); client->text(out);
...
void handleRemoveWifiNetworkCommand(AsyncWebSocketClient* client, JsonDoc& in) {
  if (!in.containsKey("index")) { sendErrorResponse(client, "Missing index"); return; }
  int index = in["index"].as<int>();

  Preferences prefs;
  if (!prefs.begin(WIFI_NAMESPACE, true)) { sendErrorResponse(client, "NVS open failed"); return; }
  String json = prefs.getString(WIFI_CREDENTIALS_KEY, "[]"); prefs.end();

  JsonDoc wdoc(WIFI_CREDENTIALS_JSON_SIZE);
  if (jDeserialize(wdoc, json)) { sendErrorResponse(client, "Parse error"); return; }
  if (!wdoc.is<JsonArray>()) { sendErrorResponse(client, "Bad format"); return; }

  JsonArray nets = wdoc.as<JsonArray>();
  if (index < 0 || index >= (int)nets.size()) { sendErrorResponse(client, "Invalid index"); return; }
  nets.remove(index);
  ...
  JsonDoc resp(128);
  resp["action"]="removeWifiNetworkResponse"; resp["success"]=ok; resp["index"]=index;
  String out; jSerialize(resp, out); client->text(out);
}

void handleConnectWifiCommand(AsyncWebSocketClient* client, JsonDoc& in) {
  if (!in.containsKey("ssid")) { sendErrorResponse(client, "Missing SSID"); return; }
  ...
  if (!jDeserialize(wdoc, json) && wdoc.is<JsonArray>()) {
    for (JsonObject n : wdoc.as<JsonArray>()) {
      if (n["ssid"].as<String>() == ssid) { pwd = n["password"].as<String>(); break; }
    }
  }
  if (!pwd.length()) { sendErrorResponse(client, "Network not found"); return; }

  if (WiFi.status()==WL_CONNECTED) { WiFi.disconnect(); delay(500); }
  WiFi.mode(WIFI_STA);
  WiFi.begin(ssid.c_str(), pwd.c_str());

  int t=30;
  while (WiFi.status()!=WL_CONNECTED && t-- > 0) { delay(1000); }

  bool ok = WiFi.status()==WL_CONNECTED;
  JsonDoc resp(256);
  resp["action"]="connectWifiResponse"; resp["success"]=ok; resp["ssid"]=ssid;
  if (ok) { resp["ip"]=WiFi.localIP().toString(); resp["rssi"]=WiFi.RSSI(); updateSavedWiFiPasswordFromWiFiManager(ssid, wifiManager); }
  else { resp["error"]="Failed to connect"; }
  String out; jSerialize(resp, out); client->text(out);
}
