...
void handleWiFiMenu(const String& input, XY_SKxxx* ps) {
  if (input == "scan") {
    ...
    bool quoted = extractQuotedParameters(remainingInput, ssid, password, remainingInput);
    ...
    if (!quoted) {
      ...
      if (firstSpace > 0) {
        ...
        if (secondSpace > 0) {
          ...
          remainingInput = remainingInput.substring(secondSpace + 1);
        } else {
    ...
    remainingInput.trim();
    if (remainingInput.length() > 0 && isDigit(remainingInput.charAt(0))) {
      priority = remainingInput.toInt();
    }
